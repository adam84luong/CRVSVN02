...
namespace Payjr.Core.ServiceCommands.PrepaidCard
{
    public class CardActivationServiceCommand : ProviderServiceCommandBase<CardActivationRequest, CardActivationResponse>
    {
        ...
        List<CardActivationRequest> cardActivationRequest;
        ...
        {
            foreach (CardActivationRequest cardActivation in cardActivationRequest)
            {
                response.CardActivations.Add
                (
                    new CardActivationRecord
                    {
                        ActingUserIdentifier = new UserIdentifier(prepaidCardAcount.UserID.Value).Identifier,
                        ActivationSuccessful = prepaidCardAcount.VerifyAccount(new VerificationData(accountDetail), null, null, null),
                        CardIdentifier = new PrepaidCardAccountIdentifier(_prepaidCardID).ToString()
                    }
                );
            }
            return true;
        ...
        protected override void Validate(CardActivationRequest request)
        {
            ...
            var cardIdentifier = cardActivationRecord.CardIdentifier;
